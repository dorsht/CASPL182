        ...
        if ((recv_bytes = recv(client_fd,(void*)buffer,BUFF_SIZE,0))>0){
            ...
            switch(cmd->arg_count){
                case 1:
                    ...
                    break;
                case 2:
                    if (strncmp ("get",cmd->arguments[0],3)==0){
                        succ = get_helper(cmd->arguments[1]);
                    }
                    else{
                        succ = nok_helper(buffer);
                    }
...
int get_helper (char * file_name){
    if (cs->conn_state == CONNECTED){
        int file_fd;
        long size;
        if ((size = file_size(file_name))>0){
            char respond[BUFF_SIZE];
            sprintf(respond, "ok %ld\n",size);
            int bytes_send = send(client_fd, (void*)respond, strlen(respond),0); 
            if (bytes_send<1){
                return -1;
            }
            ...
            if((file_fd = open(file_name,O_RDONLY))>=0){
                ...
                int out_fd = client_fd;
                while ((bytes_sent = sendfile(out_fd,file_fd,NULL,BUFF_SIZE))>0&&size>0){
                    size-=bytes_sent;
                }
                close(file_fd);
                int recv_bytes = 0;
                ...
                if ((recv_bytes = recv(client_fd,(void*)buffer,BUFF_SIZE,0))>0){
                    cmd_line * done = parse_cmd_lines(buffer);
                    if ((strncmp(done->arguments[0],"done",4)==0)){
                        cs->conn_state = CONNECTED;
                        char * ok = "ok\n";
                        ...
                        send(client_fd, "nok\n", 4, 0);
                        return nok_helper(buffer); //shouldnt be buffer... for now 
                    }
                }
                else{
                    send(client_fd, "nok\n", 4 ,0);
