...
void section_index_helper (int x){
    if (x == SHN_UNDEF){
        printf("%-15s","UND");
    }
    else if (x == SHN_ABS){
...
void print_symbols(){
    if (fd!=-1 && header!=NULL){
        printf("%-7s %-15s %-15s %-40s %-10s \n","[Num]","Value", "Section Index", "Section Name", "Symbol Name");
        int i, j;
        int offset = header->e_shoff;
        int next = 0;
        Elf64_Sym * sym = NULL;
        Elf64_Shdr * section_header = (Elf64_Shdr*)(map_start+offset), * temp = NULL;
        str_table = (char *)(map_start+((Elf64_Shdr *)(map_start+offset+(sizeof(Elf64_Shdr)*((Elf64_Ehdr *)map_start)->e_shstrndx)))->sh_offset);
        for (i=0; i<header->e_shnum; i++){ 
            temp = (Elf64_Shdr *)(map_start+offset+next);
            if (temp->sh_type == SHT_SYMTAB || temp->sh_type == SHT_DYNSYM){
                sym = (Elf64_Sym*)(map_start+temp->sh_offset);
                int size = temp->sh_size/temp->sh_entsize; // the size of the shdr / the size of all the entries = the number of entries
                for (j = 0;j<size; j++){
                    char * symbol_name;
                    if(sym[j].st_shndx == SHN_UNDEF || sym[j].st_shndx == SHN_ABS){
                        symbol_name = "";
                    }
                    else{
                        symbol_name = str_table + section_header[sym[j].st_shndx].sh_name;
                    }
                    printf("[%3d]%-3s",j," ");
                    printf("%015lx ",sym[j].st_value);
                    section_index_helper(sym[j].st_shndx);
                    printf("%-40s ",(char*)(map_start+section_header[temp->sh_link].sh_offset+sym[j].st_name));
                    printf("%-10s\n",symbol_name);
        ...
        printf("Error: File not initiallized\n");
    }
}
void print_selects(){
...
myELF_func arr[4] = {examine_ELF_file, print_section_names,print_symbols,quit};
